...
test("Media upload, reuse, usage guard, keyboard focus, accessibility, and responsive layout", async ({
  page,
}) => {
  ...
  try {
    ...
    await expect(
      page
        .getByRole("alert")
        .filter({ hasText: "Only JPEG, PNG, WebP, and AVIF" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("alert").filter({ hasText: "must not exceed 5 MB" }),
    ).toBeVisible();
    ...
    const pickerTrigger = page
      .getByRole("button", { name: "Choose from Media" })
      .first();
    await pickerTrigger.focus();
    await page.keyboard.press("Enter");
    await expect(
      page.getByRole("heading", { name: "Choose a featured image" }),
    ).toBeVisible();
    expect(
      await page
        .getByRole("dialog")
    ...
    const copied = await admin.storage
      .from("draft-assets")
      .list(articleFolder, { limit: 100 });
    expect(copied.error).toBeNull();
    expect(
      (copied.data ?? []).filter((item) => item.name.includes(filename)),
    ).toHaveLength(1);

    await page.goto("/admin/media");
    const inUseCard = page
      .getByRole("button", { name: new RegExp(`Select asset .*${filename}`) })
      .filter({ hasText: "In Use" });
    await expect(inUseCard).toHaveCount(1);
    await inUseCard.click();
    await expect(
      page.locator("aside").getByText(/Used in draft article/),
    ).toBeVisible();
    await expect(
      page.locator("aside").getByText("Deletion Unavailable"),
    ).toBeVisible();
    await expect(

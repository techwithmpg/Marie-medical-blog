...
test("Phase 2 local Storage supports private signed upload, exact facts, immutable reuse, and RLS", async () => {
  ...
  try {
    const prepared = await admin.storage
      .from("draft-assets")
      .createSignedUploadUrl(privateSource);
    assert.equal(
      prepared.error,
      null,
      "admin can prepare private signed upload",
    );
    assert.ok(prepared.data?.token);
    const uploaded = await admin.storage
      .from("draft-assets")
      .uploadToSignedUrl(privateSource, prepared.data.token, PNG, {
    ...
    assert.equal(
      uploaded.error,
      null,
      "signed upload succeeds without overwrite",
    );

    const publicUpload = await admin.storage
      .from("public-assets")
      .upload(publicSource, PNG, { contentType: "image/png", upsert: false });
    ...
    const privateReuse = await admin.storage
      .from("draft-assets")
      .copy(privateSource, privateCopy);
    assert.equal(privateReuse.error, null, "private-to-private copy succeeds");
    const publicReuse = await admin.storage
      .from("public-assets")
      .copy(publicSource, publicCopy, { destinationBucket: "draft-assets" });
    assert.equal(publicReuse.error, null, "public-to-private copy succeeds");

    assert.equal(
      (await admin.storage.from("draft-assets").download(privateSource)).error,
      null,
      "private source remains after copy",
    );
    assert.equal(
      (await admin.storage.from("public-assets").download(publicSource)).error,
      null,
      "public source remains after copy",
    );
    assert.equal(
      (await admin.storage.from("draft-assets").download(privateCopy)).error,
      null,
    );
    assert.equal(
      (await admin.storage.from("draft-assets").download(publicCopy)).error,
      null,
    );

    assert.ok(
      (await reader.storage.from("draft-assets").download(privateSource)).error,
      "non-admin cannot read private assets",
    );
    const deniedInventory = await reader.storage
      .from("draft-assets")
      .list("library");
    assert.equal(
      deniedInventory.error,
      null,
      "RLS-hidden listings may return an empty success response",
    );
    assert.equal(
      deniedInventory.data?.length,
      0,
      "non-admin inventory reveals no private objects",
    );
    await reader.storage.from("draft-assets").remove([privateSource]);
    assert.equal(
      (await admin.storage.from("draft-assets").download(privateSource)).error,
      null,
      "non-admin delete attempt leaves the private object intact",
    );

    const duplicate = await admin.storage
      .from("draft-assets")
      .upload(privateSource, PNG, { contentType: "image/png", upsert: false });

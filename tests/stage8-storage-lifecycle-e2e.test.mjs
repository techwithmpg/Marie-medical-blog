...
test("Real Local Storage Cross-Bucket Promotion, Demotion, Replacement, and Invariant Verification", async () => {
  ...
  let replacementDraftPath = null;
  let replacementPublicPath = null;
  ...
  try {
    ...
    const { data: unpublishData, error: unpublishError } =
      await adminClient.rpc("unpublish_article", {
        p_article_id: articleId,
    ...
    replacementDraftPath = `articles/${articleId}/featured/replacement-${runId}.png`;
    replacementPublicPath = `articles/${articleId}/featured/public-replacement-${crypto.randomUUID()}.png`;
    ...
    // Cleanup is deliberately centralized in finally so a failed assertion
    // cannot leave synthetic articles or Storage objects behind.
  } finally {
    const publicPaths = [promotedPublicPath, replacementPublicPath].filter(
      (value) => typeof value === "string",
    );
    const privatePaths = [
      initialDraftPath,
      demotedPrivatePath,
      replacementDraftPath,
    ].filter((value) => typeof value === "string");

    if (publicPaths.length > 0) {
      await adminClient.storage.from("public-assets").remove(publicPaths);
    }

    if (privatePaths.length > 0) {
      await adminClient.storage.from("draft-assets").remove(privatePaths);
    }

    const { error: cleanupArticleError } = await adminClient
      .from("articles")
      .delete()
      .eq("id", articleId);
    ...
    assert.equal(
      cleanupArticleError,
      null,
      "Synthetic storage lifecycle article cleanup must succeed",
    );
  }
});

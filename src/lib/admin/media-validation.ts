...
export function sanitizeFilename(rawFilename: string): string {
  ...
  return extension ? `${stem.slice(0, 220)}.${extension.slice(0, 16)}` : stem.slice(0, 237);
}
...
export const copyMediaToArticleSchema = z.object({
  articleId: z
    .string()
    .trim()
...
export const compensateImageSchema = z.object({
  articleId: z
    .string()
    .trim()
    .regex(UUID_REGEX, "Invalid article ID format."),

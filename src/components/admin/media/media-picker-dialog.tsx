...
export function MediaPickerDialog({
  ...
  const load = React.useCallback(async () => {
    ...
    setSelectedId(
      result.items.find((item) => item.isEligibleForReuse)?.id ?? null,
    );
  }, []);
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="bg-reading-surface fixed top-1/2 left-1/2 z-50 flex max-h-[88vh] w-[94vw] max-w-4xl -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border border-subtle-divider p-5 shadow-xl">
          <div className="flex items-start justify-between gap-4 border-b border-subtle-divider pb-4">
            <div>
              ...
              <Dialog.Description className="mt-1 text-sm text-ink-muted">
                A private article-owned copy will be created. Enter contextual
                alt text after selection.
              </Dialog.Description>
            </div>
            <Dialog.Close
              aria-label="Close Media picker"
              className="grid size-11 shrink-0 cursor-pointer place-items-center rounded-md text-ink-muted hover:bg-subtle-field focus-visible:ring-2 focus-visible:ring-focus-slate focus-visible:outline-none"
            >
          ...
          <div className="min-h-48 flex-1 overflow-y-auto py-4">
            {loading ? (
              <div
                className="flex min-h-48 items-center justify-center gap-2 text-sm text-ink-muted"
                aria-live="polite"
              >
              ...
              <p className="rounded-md border border-dashed border-subtle-divider p-8 text-center text-sm text-ink-muted">
                No reusable images are available.
              </p>
            ) : (
              <div
                ...
                {items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        item.isEligibleForReuse && setSelectedId(item.id)
                      }
                      className={cn(
                        ...
                        chosen
                          ? "border-oxide ring-1 ring-oxide"
                          : "border-subtle-divider",
                        !item.isEligibleForReuse &&
                    ...
                    >
                      <span className="relative block aspect-square bg-subtle-field">
                        ...
                        {chosen && (
                          <Check
                            className="absolute top-2 right-2 size-6 rounded-full bg-oxide p-1 text-paper"
                            aria-hidden="true"
                          />
                        )}
                      </span>
                      <span className="block truncate p-2 text-xs font-medium text-ink">
                        {item.name}
                      </span>
                      {!item.isEligibleForReuse && (
                        <span className="block px-2 pb-2 text-[0.6875rem] text-destructive">
          ...
          <div className="flex flex-wrap justify-end gap-3 border-t border-subtle-divider pt-4">
            <Dialog.Close className="min-h-11 cursor-pointer rounded-md border border-control-border px-4 text-sm font-semibold text-ink focus-visible:ring-2 focus-visible:ring-focus-slate focus-visible:outline-none">
              Cancel
            </Dialog.Close>
            <button
              type="button"
              onClick={handleChoose}
              disabled={!selected?.isEligibleForReuse || copying}
              className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-md bg-ink px-4 text-sm font-semibold text-paper focus-visible:ring-2 focus-visible:ring-focus-slate focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
            >
              {copying && (

...
export function MediaManager({ initialMedia }: MediaManagerProps) {
  ...
  const [deleteTarget, setDeleteTarget] =
    React.useState<AdminMediaRecord | null>(null);
  ...
  const selectedAsset =
    mediaList.find((item) => item.id === selectedId) ?? null;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      setUploadError(
        "Only JPEG, PNG, WebP, and AVIF image formats are permitted.",
      );
    ...
    try {
      ...
      if (!prepResult.success || !prepResult.path || !prepResult.token) {
        setUploadError(
          prepResult.error || "Failed to prepare upload destination.",
        );
        return;
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (selectedId === deleteTarget.id) {
        const remaining = mediaList.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 border-b border-subtle-divider pb-5 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            Private-by-default image library for editorial draft authoring and
            featured-image reuse.
          </p>
        </div>
        ...
        <div>
          ...
          <label
            ...
            className={cn(
              "hover:bg-oxide-muted inline-flex min-h-[44px] cursor-pointer items-center gap-2 rounded-md bg-oxide px-4 py-2 text-sm font-semibold text-paper shadow-xs transition-colors focus-visible:ring-2 focus-visible:ring-focus-slate focus-visible:outline-none",
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-subtle-divider pb-3">
        ...
        >
          ...
          Private Drafts (
          {mediaList.filter((i) => i.bucket === "draft-assets").length})
        </button>
        <button
          type="button"
        ...
        >
          <Globe className="text-teal-accent size-3" />
          Public Published (
          {mediaList.filter((i) => i.bucket === "public-assets").length})
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-8">
          {filteredMedia.length === 0 ? (
            <div className="bg-reading-surface flex flex-col items-center justify-center rounded-lg border border-dashed border-subtle-divider p-12 text-center">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? "border-oxide shadow-xs ring-1 ring-oxide"
                    ...
                    <div className="relative aspect-square w-full bg-subtle-field">
                      {asset.previewUrl ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center p-2 text-center text-ink-muted">
                          ...
                          <span className="mt-1 text-[0.625rem]">
                            Preview unavailable
                          </span>
                        </div>
                      )}
                      ...
                      <div className="absolute top-1.5 left-1.5 flex flex-wrap gap-1">
                        {asset.isPrivate ? (
                          ...
                          <span
                            title="Public asset"
                            className="bg-teal-accent/90 inline-flex items-center gap-0.5 rounded-xs px-1.5 py-0.5 text-[0.5625rem] font-semibold text-paper backdrop-blur-xs"
                          >
        ...
        <aside
          ...
          className="bg-reading-surface hidden rounded-lg border border-subtle-divider p-5 shadow-xs lg:col-span-4 lg:block"
        >
          {selectedAsset ? (
            <div className="space-y-5">
              ...
              <div className="space-y-2.5 text-xs">
                <div>
                  <span className="font-semibold text-ink-muted uppercase">
                    Filename
                  </span>
                  <p className="font-mono break-all text-ink">
                    {selectedAsset.name}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2 rounded-md bg-paper p-2 font-mono text-[0.6875rem] text-ink">
                    <span className="truncate">
                      {selectedAsset.bucket}/{selectedAsset.path}
                    </span>
                    <button
                      type="button"
                  ...
                  <div>
                    ...
                    <p className="text-ink">
                      {selectedAsset.createdAt
                        ? formatAdminDate(selectedAsset.createdAt)
                        : "—"}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-md border border-subtle-divider bg-paper p-3">
                  <span className="block font-semibold text-ink uppercase">
                    Usage Status
                  </span>
                  {!selectedAsset.usage.canDelete ? (
                    <p className="mt-1 font-medium text-oxide">
                      {selectedAsset.usage.locationDescription ||
                        "Referenced by publication"}
                    </p>
                  ) : (
                    <p className="mt-1 text-ink-muted">
                      Unused — available for selection or removal
                    </p>
                  )}
                </div>
                ...
                <div className="rounded-md border border-subtle-divider bg-paper p-3">
                  ...
                  {selectedAsset.isEligibleForReuse ? (
                    ...
                    <p className="mt-1 flex items-start gap-1 font-medium text-warning">
                      ...
                      <span>
                        {selectedAsset.ineligibilityReason ||
              ...
              <div className="pt-2">
                {!selectedAsset.usage.canDelete ? (
                  <div className="rounded-md bg-subtle-field p-3 text-xs text-ink-muted">
                    ...
                    <p className="mt-0.5">
                      This asset is currently in use. Replace or remove its
                      reference in the article before deleting.
                    </p>
                  </div>
                ) : (
                  <button
      ...
      <Dialog.Root open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Popup
            ...
            className="bg-reading-surface fixed top-1/2 left-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-subtle-divider p-6 shadow-lg transition-transform data-ending-style:scale-95 data-starting-style:scale-95"
          >
            ...
            <Dialog.Description className="mt-2 text-xs text-ink-muted">
              ...
              from <strong className="text-ink">{deleteTarget?.bucket}</strong>?
      ...
      <Dialog.Root open={mobileDetailOpen} onOpenChange={setMobileDetailOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Popup
            ...
            className="bg-reading-surface fixed inset-x-0 bottom-0 z-50 max-h-[85vh] overflow-y-auto rounded-t-xl border-t border-subtle-divider p-6 shadow-xl lg:hidden"
          >
            {selectedAsset && (

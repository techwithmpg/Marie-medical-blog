...
- **Admin-Local Read-Only Draft Preview Modal:** Authentic full-fidelity Evidence Folio reading preview rendering the drafted ProseMirror document tree, structured references, and metadata side-by-side inside the private writer workspace only. This component is strictly admin-local and explicitly does NOT implement a public preview route, preview tokens, public draft access, publication, promotion to public-assets, or Stage-8 publishing workflows.
